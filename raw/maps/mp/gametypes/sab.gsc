...
onStartGameType()
{
	...
	level._spawn_axis = maps\mp\gametypes\_spawnlogic::getSpawnpointArray( "mp_sab_spawn_axis" );
	level._spawn_axis_planted = maps\mp\gametypes\_spawnlogic::getSpawnpointArray( "mp_sab_spawn_axis_planted" );
	level._spawn_axis_planted = array_combine( level._spawn_axis_planted, level._spawn_axis );
	
	level._spawn_allies = maps\mp\gametypes\_spawnlogic::getSpawnpointArray( "mp_sab_spawn_allies" );
	level._spawn_allies_planted = maps\mp\gametypes\_spawnlogic::getSpawnpointArray( "mp_sab_spawn_allies_planted" );
	level._spawn_allies_planted = array_combine( level._spawn_allies_planted, level._spawn_allies );
	
	level._spawn_axis_start = maps\mp\gametypes\_spawnlogic::getSpawnpointArray( "mp_sab_spawn_axis_start" );
	level._spawn_allies_start = maps\mp\gametypes\_spawnlogic::getSpawnpointArray( "mp_sab_spawn_allies_start" );
...
getSpawnPoint()
{
	...
	{
		if ( isDefined( level._bombplanted ) && level._bombplanted && ( isDefined( level._bombOwner ) && spawnTeam == level._bombOwner.team ) )
		{
			if (spawnteam == "axis")
				spawnpoint = maps\mp\gametypes\_spawnlogic::getSpawnpoint_NearTeam( level._spawn_axis_planted );
			else
				spawnpoint = maps\mp\gametypes\_spawnlogic::getSpawnpoint_NearTeam( level._spawn_allies_planted );
			
		}
		else
		{
			if (spawnteam == "axis")
				spawnpoint = maps\mp\gametypes\_spawnlogic::getSpawnpoint_NearTeam(level._spawn_axis);
			else
				spawnpoint = maps\mp\gametypes\_spawnlogic::getSpawnpoint_NearTeam(level._spawn_allies);
		}
	}
...
updateGametypeDvars()
{
	...
	level._bombTimer = dvarFloatValue( "bombtimer", 45, 1, 300 );
	level._hotPotato = dvarIntValue( "hotpotato", 1, 0, 1 );
	level._scoreMode = getWatchedDvar( "scorelimit" );
}
...
sabotage()
{
	...
	level._sabBomb = maps\mp\gametypes\_gameobjects::createCarryObject( "neutral", trigger, visuals, (0,0,32) );
	level._sabBomb maps\mp\gametypes\_gameobjects::allowCarry( "any" );
	level._sabBomb maps\mp\gametypes\_gameobjects::set2DIcon( "enemy", "waypoint_bomb" );
	level._sabBomb maps\mp\gametypes\_gameobjects::set3DIcon( "enemy", "waypoint_bomb" );
	level._sabBomb maps\mp\gametypes\_gameobjects::set2DIcon( "friendly", "waypoint_bomb" );
	level._sabBomb maps\mp\gametypes\_gameobjects::set3DIcon( "friendly", "waypoint_bomb" );
	level._sabBomb maps\mp\gametypes\_gameobjects::setCarryIcon( "hud_suitcase_bomb" );
	level._sabBomb maps\mp\gametypes\_gameobjects::setVisibleTeam( "any" );
	level._sabBomb.objIDPingEnemy = true;
	level._sabBomb.onPickup = ::onPickup;
	level._sabBomb.onDrop = ::onDrop;
	level._sabBomb.allowWeapons = true;
	level._sabBomb.objPoints["allies"].archived = true;
	level._sabBomb.objPoints["axis"].archived = true;
	level._sabBomb.autoResetTime = 60.0;
	...
	if ( game["switchedsides"] )
	{
		level._bombZones["allies"] = createBombZone( "allies", getEnt( "sab_bomb_axis", "targetname" ) );
		level._bombZones["axis"] = createBombZone( "axis", getEnt( "sab_bomb_allies", "targetname" ) );
	}
	else
	{
		level._bombZones["allies"] = createBombZone( "allies", getEnt( "sab_bomb_allies", "targetname" ) );
		level._bombZones["axis"] = createBombZone( "axis", getEnt( "sab_bomb_axis", "targetname" ) );
	}
...
getClosestSite()
{
	if ( distance2d( self.origin, level._bombZones["allies"].trigger.origin ) < distance2d( self.origin, level._bombZones["axis"].trigger.origin ) )
		return ( "allies" );
	else
		return ( "axis" );
}
...
distanceToSite( team )
{
	return ( distance2d( self.origin, level._bombZones[team].trigger.origin ) );
}

scoreThread()
{
	level._bombDistance = distance2d( getEnt( "sab_bomb_axis", "targetname" ) getOrigin(), getEnt( "sab_bomb_allies", "targetname" ) getOrigin() );

	threatDistance = (level._bombDistance/2) - 384;
	
	bombEnt = level._sabBomb.trigger;
	...
	for ( ;; )
	{	
		if ( isDefined( level._sabBomb.carrier ) )
			bombEnt = level._sabBomb.carrier;
		else
			bombEnt = level._sabBomb.trigger;
		...
		if ( bombEnt distanceToSite( "allies" ) < threatDistance )
			dangerTeam = level._bombZones["allies"] maps\mp\gametypes\_gameobjects::getOwnerTeam();
		else if ( bombEnt distanceToSite( "axis" ) < threatDistance )
			dangerTeam = level._bombZones["axis"] maps\mp\gametypes\_gameobjects::getOwnerTeam();
		else if ( bombEnt distanceToSite( "allies" ) > level._bombDistance && bombEnt getClosestSite() != "allies" )
			dangerTeam = level._bombZones["axis"] maps\mp\gametypes\_gameobjects::getOwnerTeam();
		else if ( bombEnt distanceToSite( "axis" ) > level._bombDistance && bombEnt getClosestSite() != "axis" )
			dangerTeam = level._bombZones["allies"] maps\mp\gametypes\_gameobjects::getOwnerTeam();

		if ( dangerTeam != "none" )
		{
			if ( !level._bombPlanted || !getWatchedDvar( "scorelimit" ) || (level._bombPlanted && (maps\mp\gametypes\_gamescore::_getTeamScore( level._otherTeam[dangerTeam] ) < getWatchedDvar( "scorelimit" ) - 1)) )
			{
				maps\mp\gametypes\_gamescore::_setTeamScore( level._otherTeam[dangerTeam], maps\mp\gametypes\_gamescore::_getTeamScore( level._otherTeam[dangerTeam] ) + 1 );
				maps\mp\gametypes\_gamescore::updateTeamScore( level._otherTeam[dangerTeam] );
			}
		}

		if ( dangerTeam != lastDangerTeam && !level._bombExploded )
		{
...
onPickup( player )
{
	...
	level._bombZones[team] maps\mp\gametypes\_gameobjects::setVisibleTeam( "none" );
	level._bombZones[otherTeam] maps\mp\gametypes\_gameobjects::setVisibleTeam( "any" );
...
//	printOnTeamArg( &"MP_EXPLOSIVES_DROPPED_BY", otherTeam, &"MP_THE_ENEMY" );
	...
	level._bombZones["allies"] maps\mp\gametypes\_gameobjects::setVisibleTeam( "none" );
	level._bombZones["axis"] maps\mp\gametypes\_gameobjects::setVisibleTeam( "none" );		
}
...
onUse( player )
{
	...
	if ( !self maps\mp\gametypes\_gameobjects::isFriendlyTeam( player.pers["team"] ) )
	{
		...
		level._sabBomb.autoResetTime = undefined;
		level._sabBomb maps\mp\gametypes\_gameobjects::allowCarry( "none" );
		level._sabBomb maps\mp\gametypes\_gameobjects::setVisibleTeam( "none" );
		level._sabBomb maps\mp\gametypes\_gameobjects::setDropped();
	...
	else // defused the bomb
	{
		...
		if ( isDefined( level._bombOwner ) && ( level._bombOwner.bombPlantedTime + 3000 + (level._defuseTime*1000) ) > getTime() && isReallyAlive( level._bombOwner ) )
		...
		level._sabBomb maps\mp\gametypes\_gameobjects::allowCarry( "any" );
		level._sabBomb maps\mp\gametypes\_gameobjects::setPickedUp( player );
	}
}
...
bombPlanted( destroyedObj, team )
{
	...
	setGameEndTime( int( getTime() + (level._bombTimer * 1000) ) );
	...
	if ( !level._bombPlanted )
	{
		if ( level._hotPotato )
		{
			...
			level._bombTimer -= timePassed;
		}
		return;
	}

	explosionOrigin = level._sabBomb.visuals[0].origin;
	level._bombExploded = true;	
	setDvar( "ui_danger_team", "BombExploded" );
	
	if ( isdefined( level._bombowner ) )
	{
		destroyedObj.visuals[0] radiusDamage( explosionOrigin, 512, 200, 20, level._bombowner );
		level._bombowner incPlayerStat( "targetsdestroyed", 1 );
	}
	else
	...
	if ( level._scoreMode )
		maps\mp\gametypes\_gamescore::_setTeamScore( team, int( max( getWatchedDvar( "scorelimit" ), maps\mp\gametypes\_gamescore::_getTeamScore( level._otherTeam[team] ) + 1 ) ) );
	else
	...
	if ( isDefined( level._bombOwner ) )
	{
		level._bombOwner thread maps\mp\gametypes\_rank::giveRankXP( "destroy" );
		maps\mp\gametypes\_gamescore::givePlayerScore( "destroy", level._bombOwner );		
		level thread teamPlayerCardSplash( "callout_destroyed_objective", level._bombOwner );
	}
...
bombTimerWait()
{
	...
	maps\mp\gametypes\_hostmigration::waitLongDurationWithGameEndTimeUpdate( level._bombTimer );
}
...
bombDistanceThread()
{
	...
	if ( cointoss() )
		level._dangerTeam = "allies";
	else
		level._dangerTeam = "axis";
	...
	for ( ;; )
	{
		if ( isDefined( level._sabBomb.carrier ) )
			...
			bombEnt = level._sabBomb.visuals[0];

		if ( distance( bombEnt.origin, level._bombZones[getOtherTeam(level._dangerTeam)].visuals[0].origin ) < distance( bombEnt.origin, level._bombZones[level._dangerTeam].visuals[0].origin ) )
			level._dangerTeam = getOtherTeam( level._dangerTeam );
...
resetBombsite()
{
	...
	self maps\mp\gametypes\_gameobjects::setKeyObject( level._sabBomb );
...
bombDefused( object )
{
	...
	level._scoreLimitOverride = false;
