...
onSpawnPlayer()
{
	...
	if ( level._multiBomb && !isDefined( self.carryIcon ) && self.pers["team"] == game["attackers"] && !level._bombPlanted )
	{
		if ( level._splitscreen )
		{
...
updateGametypeDvars()
{
	...
	level._multiBomb = dvarIntValue( "multibomb", 0, 0, 1 );
}
...
bombs()
{
	...
	if ( !level._multiBomb )
	{
		level._sdBomb = maps\mp\gametypes\_gameobjects::createCarryObject( game["attackers"], trigger, visuals, (0,0,32) );
		level._sdBomb maps\mp\gametypes\_gameobjects::allowCarry( "friendly" );
		level._sdBomb maps\mp\gametypes\_gameobjects::set2DIcon( "friendly", "waypoint_bomb" );
		level._sdBomb maps\mp\gametypes\_gameobjects::set3DIcon( "friendly", "waypoint_bomb" );
		level._sdBomb maps\mp\gametypes\_gameobjects::setVisibleTeam( "friendly" );
		level._sdBomb maps\mp\gametypes\_gameobjects::setCarryIcon( "hud_suitcase_bomb" );
		level._sdBomb.allowWeapons = true;
		level._sdBomb.onPickup = ::onPickup;
		level._sdBomb.onDrop = ::onDrop;
	}
	else
	{
		trigger delete();
	...
	for ( index = 0; index < bombZones.size; index++ )
	{
		...
		if ( !level._multiBomb )
			bombZone maps\mp\gametypes\_gameobjects::setKeyObject( level._sdBomb );
...
onBeginUse( player )
{
	if ( self maps\mp\gametypes\_gameobjects::isFriendlyTeam( player.pers["team"] ) )
	{
		...
		if ( isDefined( level._sdBombModel ) )
			level._sdBombModel hide();
	}
	else
	{
...
onEndUse( team, player, result )
{
	...
	if ( self maps\mp\gametypes\_gameobjects::isFriendlyTeam( player.pers["team"] ) )
	{
		if ( isDefined( level._sdBombModel ) && !result )
		{
			level._sdBombModel show();
		}
	}
	else
	{
		if ( level._multibomb && !result )
		{
			for ( i = 0; i < self.otherBombZones.size; i++ )
			{
...
onUsePlantObject( player )
{
	...
	if ( !self maps\mp\gametypes\_gameobjects::isFriendlyTeam( player.pers["team"] ) )
	{
		...
		for ( index = 0; index < level._bombZones.size; index++ )
		{
			if ( level._bombZones[index] == self )
				continue;
				
			level._bombZones[index] maps\mp\gametypes\_gameobjects::disableObject();
		}
...
bombPlanted( destroyedObj, player )
{
	...
	if ( !level._multiBomb )
	{
		level._sdBomb maps\mp\gametypes\_gameobjects::allowCarry( "none" );
		level._sdBomb maps\mp\gametypes\_gameobjects::setVisibleTeam( "none" );
		level._sdBomb maps\mp\gametypes\_gameobjects::setDropped();
		level._sdBombModel = level._sdBomb.visuals[0];
	}
	else
	{
		...
		{
			if ( isDefined( level._players[index].carryIcon ) )
				level._players[index].carryIcon destroyElem();
		}
		...
		level._sdBombModel = spawn( "script_model", trace["position"] );
		level._sdBombModel.angles = dropAngles;
		level._sdBombModel setModel( "prop_suitcase_bomb" );
	}
	...
	*/
	...
	trigger.origin = level._sdBombModel.origin;
	visuals = [];
	defuseObject = maps\mp\gametypes\_gameobjects::createUseObject( game["defenders"], trigger, visuals, (0,0,32) );
	defuseObject maps\mp\gametypes\_gameobjects::allowUse( "friendly" );
	defuseObject maps\mp\gametypes\_gameobjects::setUseTime( level._defuseTime );
	...
	if ( level._gameEnded || level._bombDefused )
		return;
	
	level._bombExploded = true;
	
	explosionOrigin = level._sdBombModel.origin;
	level._sdBombModel hide();
